...
parser.add_argument("--blobname", type=str)
...
try:
        ...
        extraced_json = analyze_invoice(invoices, blob_name)
        print(extraced_json)

        with open(f"{output_path}/{blob_name}.json", 'w') as f2: 
            json.dump(extraced_json, f2)


        print(f"The document {blob_name} completed:")

except Exception as e:
        
        with open(f"{output_path}/{blob_name}.json", 'w') as f2: 
            json.dump({"file_name": blob_name, "documents":[]}, f2)
   
        print(f"The {blob_name} document failed:")
        pass